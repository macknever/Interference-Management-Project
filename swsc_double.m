...
for column = 1:length(rate_list)
    ...
    enc = swsc_whole_encoding(m1,info_len,rate_1,cbsInfo_1);
    ...
    [Y1_hat,Y2_hat] = correction(Y1,Y2,n,info_len,rate_1,rate_2,cbsInfo_1,cbsInfo_2,g1,g2,outlen,alpha);
...
function [Y1_hat,Y2_hat] = correction(Y1,Y2,n,info_len,rate_1,rate_2,cbsInfo_1,cbsInfo_2,g1,g2,outlen,alpha)%y is transmitted super_position, H the the first n bits u
    ...
    for blk = 1:b-1
        ...
        x2_hat = decoding(x2,outlen,rate_2,cbsInfo_2);
        Y2_hat = cat(2,Y2_hat,x2_hat);
        x2_hat = encoding(x2_hat,info_len/2,rate_2,cbsInfo_2);
        
        v_1 = Y1(n*blk-n+1:n*blk,1)-2*alpha*u;
        v_1 = v_1-g2*x2_hat;
        y_2 = Y1(blk*n+1:(blk+1)*n);
        y = cat(1,v_1,y_2);
        y_hat = decoding(y,outlen,rate_1,cbsInfo_1);
        Y1_hat = cat(2,Y1_hat,y_hat);
        u = encoding(y_hat,info_len,rate_1,cbsInfo_1);
        u = snd(u);
    end
end
